...
class Loss(nn.Module):
    ...
    def get_cd_loss(self,pred, gt, pcd_radius):
        ...
        cost = cost_for + cost_bac
        cost /= pcd_radius
        cost = torch.mean(cost)
        return cost

    def get_cd_loss2(self,pred, gt, pcd_radius):
        cost_for, cost_bac = chamfer_distance(gt, pred)
        cost = torch.mean(cost_for) + torch.mean(cost_bac)
        cost /= pcd_radius
        return cost

    def get_hd_loss(self,pred,gt,pcd_radius):
        cost_for, cost_bac = chamfer_distance(gt, pred)
        maxf,_ = torch.max(cost_for, 1,keepdim=False)
        maxb,_ = torch.max(cost_bac, 1,keepdim=False)
        costf = torch.mean(maxf)
        costb = torch.mean(maxb)
        if costf>costb:
            return costf
        else:
            return costb
    ...
    def loss_fn_kd(self,outputs, labels, teacher_outputs, alpha,temperature):
        """
    ...
    def corss_entropy(self,output,label):
        return F.cross_entropy(output,label)

    def corss_entropy2(self,output,label):
        return F.cross_entropy(output,label)
...
if __name__=="__main__":
    ...
    random.seed(seed)
    loss=Loss().cuda()
    point_cloud=torch.rand(4,1024,3).cuda()
    point_cloud2 = torch.rand(4, 1024, 3).cuda()
    emd = loss.get_cd_loss(point_cloud,point_cloud2,1)
    print(emd)
