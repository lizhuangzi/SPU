...
from network.SPUN16x import Generator
...
def infer(model, G_model, test_loader, criterion, opt):
    ...
    with torch.no_grad():
        for i, (data, gt, label) in enumerate(tqdm(test_loader)):
            ...
            #lr_logits, lr_gbf_hl, _ = model(F.upsample_nearest(data,scale_factor=8))
            #lr_logits, lr_gbf_hl, _ = model(data)

            preds, feature_before_upsampling = G_model(data)  # points.shape[1])
            # classification
            #_, gbf, maxbefore_feaute = model(preds)
            #preds = G_model.halfforwad(data, feature_before_upsampling, gbf)
            sr_logits, gbf2, _ = model(preds)

            loss = criterion(sr_logits, label.squeeze())
            prec1, prec5 = accuracy(sr_logits, label, topk=(1, 5))
            top5.update(prec5.cpu().numpy(), data.size(0))

            pred = sr_logits.max(dim=1)[1]
            ...
            gt = gt.permute(0, 2, 1)
            #data = F.upsample_nearest(data, scale_factor=16)
            #data = data.permute(0,2,1)
            emd = Loss_fn.get_emd_loss(preds, gt, 1.0)
            #cd = Loss_fn.get_cd_loss(data, gt, 1.0)
            dist1, dist2 = chamfer_distance(preds,gt)
    ...
    return overall_acc, top5.avg, class_acc, meanemd, meancd, meanfenu, meanfcos, opt
...
if __name__ == '__main__':
    ...
    eval_dst = ModelNet40spu(data_dir=params["dataset_dir"], partition='test', num_points=1024, upscalefactor=4)
    ...
    G_model.load_state_dict(torch.load('../savedModel/SPUN_pointNet4x(1).parm'))
    G_model.cuda()

    criterion = nn.CrossEntropyLoss()
    opt.test_losses = AverageMeter()
    top1, top5, test_class_acc, meanemd, meancd, meanfenu, meanfcos, opt = infer(model, G_model, test_loader, criterion,
                                                                                 opt)
