...
from data.ModelNet40 import ModelNet40,ModelNet40_LR
...
def train(model, train_loader, test_loader, opt):
    ...
    for e in range(opt.epochs):
        ...
        if best_test_overall_acc<test_overall_acc_top1:
            ...
            torch.save(model.state_dict(), '../savedModel/PointNetModelNet40_1024.parm')

        out_path = '../statistics/'
        data_frame = pd.DataFrame(
            data={'Top1acc': ValEXPresult['Top1acc'], 'Top5acc': ValEXPresult['Top5acc'], 'Clsacc': ValEXPresult['Clsacc']},
            index=range(1, globalepoch + 1))
        data_frame.to_csv(out_path + 'PointNet_ModelNet40cls_1024' + '.csv', index_label='Epoch')
...
if __name__ == '__main__':
    ...
    # train_loader = DataLoader(ModelNet40_LR(data_dir=opt.data_dir, partition='train', num_points=opt.num_points//8),
    #                           num_workers=8, batch_size=opt.batch_size, shuffle=True, drop_last=True)
    # test_loader = DataLoader(ModelNet40_LR(data_dir=opt.data_dir, partition='test', num_points=opt.num_points//8),
    #                          num_workers=4, batch_size=opt.test_batch_size, shuffle=True, drop_last=False)
    # train_loader = DataLoader(ModelNet40spu(data_dir=opt.data_dir, partition='train', num_points=1024,upscalefactor=4),
    #                           num_workers=8, batch_size=opt.batch_size, shuffle=True, drop_last=True)
    # test_loader = DataLoader(ModelNet40spu(data_dir=opt.data_dir, partition='test', num_points=1024,upscalefactor=4),
