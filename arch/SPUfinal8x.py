...
class denseconv(nn.Module):
    ...
    def forward(self, input):
        ...
        # expandresult = expandresult.view(B,self.growth_rate,3,K,N)
...
class SEPS(nn.Module):
    def __init__(self,inchannel,outchannel = 256,up_ratio=4):
        ...
        self.convspe3 = Conv1d(in_channels=outchannel, out_channels=2 * outchannel, kernel_size=1)
...
class Generator(nn.Module):
    ...
    def forward(self, input,lr_jacobian):
        ...
        return coord + F.upsample_nearest(input,scale_factor=8)

    def forward2(self, input,lr_jacobian):
        lr_jacobian = F.softmin(lr_jacobian.mean(dim=1,keepdim=True), dim=2)
        #lr_jacobian = F.softmax(self.convs1(lr_jacobian), dim=1)
        features = self.feature_extractor(input)  #b,648,n # b,648,n
        #jocb = self.feature_extractor2(input*lr_jacobian)
        ...
        coord = self.conv1(upf)
        coord = self.conv2(coord)
        coord = coord + F.upsample_nearest(input, scale_factor=8)
        coord = coord.permute(0, 2, 1).contiguous()
        sampledidx = farthest_point_sample(coord, 1024)
        coord = index_points(coord,sampledidx)
        return coord.permute(0, 2, 1).contiguous()
