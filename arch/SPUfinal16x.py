...
class SEPS(nn.Module):
    def __init__(self,inchannel,outchannel = 256,up_ratio=4):
        ...
        self.convspe4 = Conv1d(in_channels=outchannel, out_channels=2 * outchannel, kernel_size=1)
...
class Generator(nn.Module):
    ...
    def forward(self, input,lr_jacobian):
        ...
        upf = self.upsampling(features+features*lr_jacobian) # b,128,4*n

        coord = self.conv1(upf)
        coord = self.conv2(coord)
        return coord + F.upsample_nearest(input,scale_factor=16)
